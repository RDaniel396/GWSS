...
type = input("Input number of model required: 1. pop III, 2. delay, 3. no delay")
if type != (1 or 2 or 3):
    print("Invalid option, use of defult model pop III")
if type ==3:
...
title=data[:,0].size
plt.figure()
plt.errorbar(data[:,0],data[:,3]*c/(3.08567758149137e22),data[:,14]*c/(3.08567758149137e22),fmt='.')
plt.xlabel('z')
plt.ylabel('$d_l [Mpc]$')
...
# np.savetxt('GWSS_data_LambdaCDM_plik.dat',temp_data)
